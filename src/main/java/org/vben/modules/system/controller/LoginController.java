...
public class LoginController {
	...
	public Result<String> sms(@RequestBody JSONObject jsonObject,HttpServletRequest request) {
		...
		try {
			...
			Map<String,Object>	b1 = DySmsHelper.sendSms(mobile, obj, DySmsEnum.REGISTER_TEMPLATE_CODE);
				b = (Boolean) b1.get("result");
			}else {
				//登录模式，校验用户有效性
				SysUser sysUser = sysUserService.getUserByPhone(mobile);
				result = sysUserService.checkUserIsEffective(sysUser);
				...
				if (CommonConstant.SMS_TPL_TYPE_0.equals(smsmode)) {
					//登录模板
				Map<String,Object>	b1 = DySmsHelper.sendSms(mobile, obj, DySmsEnum.LOGIN_TEMPLATE_CODE);
					b = (Boolean) b1.get("result");
				} else if(CommonConstant.SMS_TPL_TYPE_2.equals(smsmode)) {
					//忘记密码模板
				Map<String,Object>	b1 = DySmsHelper.sendSms(mobile, obj, DySmsEnum.FORGET_PASSWORD_TEMPLATE_CODE);
					b = (Boolean) b1.get("result");
				}
			}
	...
	public Result<String> sendSms(@RequestBody JSONObject jsonObject) {
		...
		obj.put("code", captcha);
		try {
			Map<String,Object> b1 = DySmsHelper.sendSms(mobile, obj, DySmsEnum.CHANGE_PASSWORD_TEMPLATE_CODE);
		Boolean	b = (Boolean) b1.get("result");
			if (!b) {
				result.setMessage("短信验证码发送失败,请稍后重试");
				result.setSuccess(false);
				return result;
